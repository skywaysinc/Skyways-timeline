...
while True:
    url = f"https://api.airtable.com/v0/{base_id}/{table_id}?fields%5B%5D=Flight%20%23"
    if offset:
        url += f"&offset={offset}"
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            for r in records:
                num = r.get("fields", {}).get("Flight #", 0)
                if isinstance(num, int) and num > max_flight_num:
                    max_flight_num = num
...
print(f"Max Flight Number: {max_flight_num}")
