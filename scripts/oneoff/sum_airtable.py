...
while True:
    url = f"https://api.airtable.com/v0/{base_id}/{table_id}?fields%5B%5D=Flight%20time%20(h)&fields%5B%5D=Dist%20Cruise%20(km)"
    if offset:
        url += f"&offset={offset}"
    ...
    try:
        with urllib.request.urlopen(req) as response:
            ...
            for r in records:
                fields = r.get("fields", {})
                total_hours += fields.get("Flight time (h)", 0)
                total_dist += fields.get("Dist Cruise (km)", 0)
